...
export function ProfileCard() {
  ...
  return <div className="flex flex-col relative w-full">
    ...
    <div className="absolute w-full h-[100px] z-10 top-[50px]" style={{ background: 'linear-gradient(180deg, rgba(0, 0, 0, 0.00) 0%, #000 100%)' }} />
    <div className="flex absolute gap-[12px] items-start">
      <div className="flex items-center gap-[8px]">
        <div className="rounded-[99px] border-[2px] border-white relative w-[64px] h-[64px]">
          <Image src={data?.twitter_image ? data?.twitter_image : "/mask-network.svg"} alt="avatar" layout="fill" objectFit="cover" />
        </div>
        <div className="text-white text-[32px] font-bold leading-[40px] -mt-[30px]">
          {data?.twitter_display_name ? data.twitter_display_name : address && formatAddress(address as string)}
        </div>
      </div>
    </div>
    <div className="rounded-[16px] h-[100px] flex justify-end items-end p-[16px] flex-col" style={{ background: 'var(--line-purple)' }}>
